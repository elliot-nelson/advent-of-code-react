...
class Puzzle extends Component {
    ...
    return (
      <div className="puzzle-2020-03">
        ...
        <div className="floater">
          {this.state.trees &&
          <div className="solution part1">{this.state.trees} trees</div>
          }
          {this.state.trees &&
          <div className="solution part2">{this.state.trees} trees</div>
          }
        </div>
      </div>
    )
  ...
  async run() {
    ...
    this.frame = 0;
    this.solvePart1 = solve(this.map, { x: 0, y: 0 }, { x: 3, y: 1 }, 4 * 60);
  ...
  async update(t) {
    ...
    if (result.value) {
      ...
      this.setState({ trees: result.value.trees });
  ...
  draw() {
    ...
    let offset = {
      x: this.width / 2 - this.pos.x * 8,
      y: this.height / 2 - this.pos.y * 8
    };
    ...
    if (this.pos) {
      ...
      for (let y = this.pos.y - sHeight | 0; y < this.pos.y + sHeight | 0; y++) {
        if (y >= 0 && y < this.map.length) {
          for (let x = this.pos.x - sWidth | 0; x < this.pos.x + sWidth | 0; x++) {
      ...
      this.ctx.moveTo(0, 0);
      this.ctx.lineTo(this.pos.x * 8, this.pos.y * 8);
      this.ctx.stroke();
      ...
      for (let y = this.pos.y - sHeight | 0; y < this.pos.y + sHeight | 0; y++) {
        if (y >= 0 && y < this.map.length) {
          for (let x = this.pos.x - sWidth | 0; x < this.pos.x + sWidth | 0; x++) {
            ...
            let collide = this.collisions[`${x},${y}`];
            if (collide) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(0, 0);
      this.ctx.lineTo(result.value.pos.x, result.value.pos.y);
      this.ctx.stroke();
      this.ctx.fillStyle = 'white';
      this.ctx.fillText(JSON.stringify(result.value.pos) + ", " + this.frame, 10, 90);*/
  }
}
